...
> `08 PROMPTS/Library/` (in this Laptop Sync vault)
>
> - [[00-Hub]] — main dashboard
> - [[SCHEMA]] — the schema every note must follow
> - [[Dictionary]] — controlled vocabulary (source of truth for domains, energy, tags, etc.)
> - [[How-to-Use-Ubiquitously]] — how to use the library from any device or model
> - `08 PROMPTS/Library/Tools/` — emit scripts, search, sync-to-grok, export-for-phone, build-master-context, etc.
>
> This note is kept for historical context and the original protocol definitions. For day-to-day use and editing, go to `08 PROMPTS/Library/`.
...
**Everything put in this vault (Laptop Sync) as the universal truth:**
- `08 PROMPTS/Library/` — the atomic, maintained, Dictionary-governed, queryable source of truth (Skills/, Protocols/, Contexts/, Hubs/, Tools/, Dictionary.md, SCHEMA.md, README.md, How-to-Use-Ubiquitously.md).
- `08 PROMPTS/Prompt-Library/` — full copies of the big compiled libraries for convenience (AI_Life_Coach_Friend.md, David_AI_Prompt_Library.md, ANTI_GRAVITY_Master_Prompt.md, BE_Roleplay_Generator.md, Grok_Skills.md, Tucked_Away_AI_Skills_and_Tiny_Tools.md).
- `08 PROMPTS/Library/Tools/` — the emitter scripts (emit-skill.py, build-master-context.py, export-for-phone.py, sync-to-grok.py, etc.) so you can generate clean blocks for any AI from the Library.
- STATENOTFATE/ and MOCS/ — cross-referenced with pointers to the Library as canonical.
- The atomic versions of the David 12, SNF principles (snf-proof-registration, snf-hope-activation), MVD/floor work (mvd-anchors, floor-wins), PRS clinical (prs-safety-check), etc.

**Everything we have done here (the MOCs, Tech/Human numbered, STATENOTFATE canonicals, PWA, philosophy, profile, organization log, universal /tp /council, previous session) referenced/linked in the Library and already in this vault.**

Tiny tools: listed with paths/content from .grok/scripts and the IDE ones in the Grok_Skills.md and Tucked_Away file.

The `08 PROMPTS/Library/` folder and this universal now form the consolidated **universal truth** in the Laptop Sync vault. Open in graph for connections (links added via previous organization and this append).

See the new `08 PROMPTS/Library/` for the full living atomic system. Re-invoke prompt-surgeon skill for updates ("scour again and add to library"). The skill SKILL.md in .grok/skills/prompt-surgeon/ has the full workflow for future deep research.

This fulfills the request: scoured, found the 2 + handfuls + tiny, put in this vault (Laptop Sync) as the universal truth, consolidated skills/prompts from all AI/sources/previous, copied into the Library (atomic) and Prompt-Library (compiled references).
...
- Result: VERDICT: PASS (subagent id 019ea7c7-76eb-73e1-8711-7eca22466a20). Full checklist covered and evidenced by direct tool inspection (files, logs, processes, content comparisons):
  ...
  - Verbatim copies confirmed in this vault: 08 PROMPTS/Library/ (atomic Skills/ with new SNF terms from Dictionary, Protocols/, Contexts/, Tools/, Dictionary.md, SCHEMA.md, README.md declaring source of truth, How-to-Use-Ubiquitously.md, Hubs/00-Hub.md with audit queries), 08 PROMPTS/Prompt-Library/ (full compiled AI_Life_Coach_Friend.md, David_AI_Prompt_Library.md, ANTI_GRAVITY_Master_Prompt.md, BE_Roleplay_Generator.md, Grok_Skills.md, Tucked_Away_AI_Skills_and_Tiny_Tools.md), and the atomic versions of the David 12, SNF principles (snf-proof-registration, snf-hope-activation), MVD/floor work (mvd-anchors, floor-wins), PRS clinical (prs-safety-check), etc.
...
- All carried requests (scour, verbatim consol to vault+universal+graph, 5yr org, .git/OneDrive notes, everything consolidated, Obsidian as universal truth) complete per direct evidence. No unattempted items, no FAIL issues (minor notes on ephemeral outputs / no literal CPU strings in jsonl but process capture sufficient).
...
**This vault (Laptop Sync) and specifically `08 PROMPTS/Library/` is now the universal truth for all AI.** All prompts, skills, and systems have been pasted/consolidated here. Use the Library atomic notes + Tools/ emitters to feed any AI. The old scattered locations (OneDrive ObsidianVault, AI_Work disk folders, My Drive AI_Life_Coach_Friend, root ANTI_GRAVITY, etc.) are now secondary or historical. Update and maintain here first.