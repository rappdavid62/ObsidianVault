...
### 6. Other Tools & Agent Bridges

- **ChatGPT Custom GPTs / GPTs with memory**: Use the system guidelines inside `ChatGPT-System-Prompt.md`. Paste or upload context packs generated from `sync-all.py`.
- **Claude Projects**: Upload the whole `Library/` folder (now includes complete SNF set: mvd-anchors etc.) + your current status + a couple of key notes as project knowledge. Follow `Claude-Project-Instructions.md`.
- **Gemini (Google AI Studio / app / mobile)**: Follow `Gemini-System-Prompt.md`. Paste Master Context block or upload key files for long context.
- **Antigravity IDE**: Follow `Antigravity-Instructions.md`. The agent reads workspace files directly; runs `sync-all.py` on file save automatically via the background watcher.
- **Codex (Local Goal Runs)**: Follow `Codex-Instructions.md`. It logs goal summaries to `00-CAPTURE/App Captures/` with `source_app: Codex`.
- **Grok Build**: Follow `Grok-Build-Instructions.md`. It uses Native skills in `~/.grok/skills/`.
- **Cursor / IDE agents / Windsurf**: Point `.cursorrules` or project rules at key notes (especially master-bio + relevant skills). The `.cursorrules` is automatically generated by `sync-all.py`.
